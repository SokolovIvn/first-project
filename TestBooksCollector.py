...
class TestBooksCollector:
    ...
    def test_book_name_set_name_check_true(self, book_name):  # добавили новую книгу и проверили имя
    ...
    def test_book_rating_default_rating_true(self, book_name):  # протестировать дефолтный рейтинг у добавленной книги по имени книги
        books = BooksCollector()
        books.add_new_book(name=book_name)
        assert books.get_book_rating(book_name) == 1

    def test_book_rating_set_custom_rating_true(self, book_name):  # проверка установки кастомного рейтинга
        custom_rating = 3
        books = BooksCollector()
        books.add_new_book(name=book_name)
    ...
    def test_book_specific_rating_get_some_rating_true(self, book_name):  # проверка вывода книги с определенным рейтингом
        custom_rating = 3
        books = BooksCollector()
        books.add_new_book(name=book_name)
        books.set_book_rating(book_name, custom_rating)
    ...
    def test_del_book_in_favorites_check_del_favourite_book_true(self, book_name):  # проверяем, что книга удалена из избранного
        books = BooksCollector()
        books.add_new_book(name=book_name)
        books.add_book_in_favorites(book_name)
